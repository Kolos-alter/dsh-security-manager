...
/** Package-name shape accepted by the update/check routes (npm-safe; blocks path traversal and specs). */
const SAFE_NAME_RE = /^[a-zA-Z0-9@][a-zA-Z0-9._~/-]*$/;

/**
 * Reject package names that could smuggle a filesystem path or an arbitrary
 * pnpm spec into `pnpm add` (e.g. `..`, `../../evil`, backslashes, spaces).
 * Scoped names (`@scope/name`) are allowed.
 */
export function assertSafePackageName(packageName) {
  if (
    ...
    !SAFE_NAME_RE.test(packageName)
  ) {
    throw new Error(`invalid package name: ${JSON.stringify(packageName)}`);
  }
}
...
export async function updatePackage(home, profile, pnpmPath, packageName, version, ref) {
  ...
  let spec;
  if (info.onNpm) {
    // A bare `pnpm add <name>` treats an already-pinned dependency as "up to
    // date" (pnpm 11), so an update must pin the target version explicitly —
    // the caller's version when given, else the npm `latest` dist-tag.
    const target = version || info.latest;
    spec = target ? `${packageName}@${target}` : packageName;
  } else if (info.github) {
